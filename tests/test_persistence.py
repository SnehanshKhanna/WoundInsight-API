...
def test_persistence_and_retrieval():
    ...
    with TestClient(app) as client:
        ...
        r_post = client.post(
            ...
            data={"user_id": "test_persistence_user"}
        )
        ...
        r_get = client.get(f"/api/v1/analyses/{analysis_id}")
        assert r_get.status_code == 200
        get_data = r_get.json()
        assert get_data["analysis_id"] == analysis_id
        assert get_data["user_id"] == "test_persistence_user"
        ...
        r_list = client.get("/api/v1/analyses?limit=10&offset=0")
        ...
        # 4. Filter by user_id
        r_user_list = client.get("/api/v1/analyses?user_id=test_persistence_user")
        assert r_user_list.status_code == 200
        user_list_data = r_user_list.json()
        assert user_list_data["total"] >= 1
        assert all(a["user_id"] == "test_persistence_user" for a in user_list_data["analyses"])

        # 5. Retrieve diagnostic report image
        r_report = client.get(f"/api/v1/analyses/{analysis_id}/report")
        ...
        r_orig = client.get(f"/api/v1/analyses/{analysis_id}/image")
        assert r_orig.status_code == 200
        assert len(r_orig.content) == len(img_bytes)

        # 7. Non-existent record returns 404
        r_404 = client.get("/api/v1/analyses/nonexistent-uuid-12345")
        assert r_404.status_code == 404

        r_report_404 = client.get("/api/v1/analyses/nonexistent-uuid-12345/report")
