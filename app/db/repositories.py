...
class UserRepository:
    ...
    def create_user(user: UserModel) -> UserModel:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, (user.id, user.email, user.password_hash, user.name, user.created_at))
        logger.info(f"Registered new user account: {user.email} (id: {user.id})")
        return user

    @staticmethod
    def get_by_id(user_id: str) -> Optional[UserModel]:
        sql = "SELECT id, email, password_hash, name, created_at FROM users WHERE id = %s;"
        with get_db_connection() as conn:
    ...
    def get_by_email(email: str) -> Optional[UserModel]:
        sql = "SELECT id, email, password_hash, name, created_at FROM users WHERE LOWER(email) = LOWER(%s);"
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
...
class WoundRepository:
    ...
    def create_wound(wound: WoundModel) -> WoundModel:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, (wound.id, wound.user_id, wound.name, wound.location, wound.created_at))
    ...
    def get_by_id(wound_id: str) -> Optional[WoundModel]:
        sql = "SELECT id, user_id, name, location, created_at FROM wounds WHERE id = %s;"
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                cur.execute(sql, (wound_id,))
                row = cur.fetchone()
                if not row:
                    return None
                return WoundModel(
    ...
    def list_by_user(user_id: str) -> List[WoundModel]:
        sql = "SELECT id, user_id, name, location, created_at FROM wounds WHERE user_id = %s ORDER BY created_at DESC;"
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                cur.execute(sql, (user_id,))
                rows = cur.fetchall()
...
class AnalysisRepository:
    ...
    def save_analysis(record: AnalysisRecordModel) -> str:
        ...
        params = (
            ...
            Json(record.etiology_probabilities),
            record.wound_detected,
            ...
            record.clinician_review_flag,
            Json(record.explainability_metadata) if record.explainability_metadata else None,
    ...
    def get_by_id(analysis_id: str) -> Optional[Dict[str, Any]]:
        sql = "SELECT * FROM analyses WHERE analysis_id = %s;"
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
    ...
    def list_analyses(
        ...
        if wound_id:
            where_clauses.append("wound_id = %s")
            params.append(wound_id)

        where_str = f" WHERE {' AND '.join(where_clauses)}" if where_clauses else ""
        count_sql = f"SELECT COUNT(*) FROM analyses{where_str}"
        list_sql = f"SELECT * FROM analyses{where_str} ORDER BY created_at DESC LIMIT %s OFFSET %s;"
    ...
    def _row_to_summary(row) -> Dict[str, Any]:
        return {
            ...
            "user_id": str(row["user_id"]),
            "wound_id": str(row["wound_id"]) if row.get("wound_id") else None,
    ...
    def _row_to_dict(row) -> Dict[str, Any]:
        # psycopg2 automatically parses JSONB into Python dicts
        probs = row["etiology_probabilities"] if row["etiology_probabilities"] else {}
        expl = row["explainability_metadata"] if row["explainability_metadata"] else None
